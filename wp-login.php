...
/**
 * 确保WordPress引导程序在继续之前运行。
 * Make sure that the WordPress bootstrap has run before continuing.
 */
require( dirname(__FILE__) . '/wp-load.php' );

// 如果被迫使用SSL，则重定向到HTTPS登录
...
function login_header( $title = 'Log In', $message = '', $wp_error = null ) {
	...
	/**
     * 为了匹配上面的URL/title集合，多站点站点具有博客名称，而单个站点获得标题标题。
...
function retrieve_password() {
	...
	if ( is_multisite() ) {
		...
		/**
         * 在sanitize_option中将esc_html与blogname选项一起转义到数据库中，我们希望在电子邮件的纯文本竞技场中反转此选项。
...
if ( get_option( 'users_can_register' ) ) :
	...
	/**
     * 这个过滤器被记录在wp-includes/general-template.php
...
default:
    ...
    // 如果用户想用SSL，但会话不是SSL,强制一个安全的cookie
...
<?php		exit;
	...
	if ( $interim_login ) {
		...
		elseif ( strpos( $redirect_to, 'about.php?updated' ) )
            // 你已经成功地更新了WordPress！请登录回去看看有什么新的。
...
<?php
/**
 * 过滤是否在登录屏幕上打印到“wp_attempt_focus()”的调用。
